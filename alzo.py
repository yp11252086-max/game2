...
# Forces buttons to look like a perfectly square grid layout
st.markdown(
    """
    <style>
    div.stButton > button {
...
def handle_cell_click(r, c, mode):
    """Unified handler for clicking cells to prevent errors with disabled buttons."""
    if st.session_state.game_over or st.session_state.revealed[r][c]:
        return  # Ignore clicks if game is over or cell is already revealed
...
def reveal_cascade(r, c):
    """Automatically unfolds empty neighboring cells."""
...
def check_win_condition():
    ...
    for r in range(GRID_SIZE):
        for c in range(GRID_SIZE):
            if (r, c) not in st.session_state.mines and not st.session_state.revealed[
                r][c
            ]:
                return
...
st.write(
    f"Grid size: **{GRID_SIZE}x{GRID_SIZE}** | Total Mines: **{NUM_MINES}**"
)
...
with st.sidebar:
    ...
    st.markdown("---")
    # Using a radio selector allows smooth input switching without button crashes
    play_mode = st.radio(
        "Select Action Mode:", ["Reveal Mode ⛏️", "Flag Mode 🚩"]
    )
...
for r in range(GRID_SIZE):
    ...
    for c in range(GRID_SIZE):
        with cols[c]:
            ...
            # Render button dynamically without changing disabled parameters
            st.button(
                label,
                key=f"cell_{r}_{c}",
                type=button_type,
